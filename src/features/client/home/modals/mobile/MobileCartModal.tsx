        ...
        <div className="flex-1 overflow-y-auto px-5">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <p className="text-lg text-gray-500">{t.cartEmpty}</p>
            </div>
          ) : (
                ...
                <div key={item.id} className="bg-white rounded-xl p-2.5 shadow-lg relative">
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="text-[14px]/[17px] text-[#333333] mb-1">{getTranslated(item.product, lang)}</h4>
                        <p className="text-[13px]/[16px] text-[#666666]">{item.size} ({item.color})</p>
                        {item.print && (
                          <p className="text-[13px]/[16px]">
                            {t.print}:{" "}
                            <span className="text-[#00C6F1]">{getTranslated(item.print, lang)}</span>
                          </p>
                        )}
                      </div>
        ...
        {items.length > 0 && (
          <div className="bg-white shadow-2xl m-5 p-5 rounded-xl space-y-2.5">
            <div className="space-y-2.5 text-[#333333]">
              <h3 className="text-[22px]/[27px]">{t.payment}</h3>
              ...
              <div className="flex gap-2.5 items-center">
                <span className="text-[13px]/[22px]">{t.total}:</span>
