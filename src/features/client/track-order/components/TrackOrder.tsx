    ...
    <div className="min-h-screen bg-[#FDFCFE] py-12 px-6 flex flex-col">
      <div className="max-w-4xl w-full mx-auto my-auto">
        ...
        <div className="relative text-center mb-16 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <NextLink
            href="/"
            className="inline-block mb-10 transform hover:scale-105 transition-transform"
          >
            <Image
              src="/art-lavka.png"
              alt="ART LAVKA"
        ...
        {!order && ordersList.length === 0 && (
          <div className="bg-white rounded-sm p-2.5 md:p-5 shadow-[0_40px_80px_-16px_rgba(0,0,0,0.06)] border border-gray-50 animate-in fade-in zoom-in-95 duration-500">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {searchMode === "order-number" && (
                  <div className="space-y-3">
                    <label className="block text-sm font-semibold tracking-widest text-gray-400 ml-1">
                      {t.yourOrderNumber}
                    </label>
                    <div className="relative group">
                      <MdInfoOutline
                        className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#8814B1] transition-colors"
                        size={20}
                      />
                      <input
                        type="text"
                        ...
                        className="w-full pl-14 pr-6 py-2.5 bg-gray-50 border border-gray-100 rounded-sm focus:bg-white focus:ring-4 focus:ring-purple-50 focus:border-[#8814B1] outline-none transition-all font-black text-gray-900 uppercase placeholder:text-gray-300 tracking-wider"
                ...
                <div
                  className={`${searchMode === "phone-only" ? "md:col-span-2" : ""} space-y-3`}
                >
                  <label className="block text-sm font-semibold tracking-widest text-gray-400 ml-1">
                    {t.phoneNumberLabel}
                  </label>
                  <div className="relative group">
                    <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 font-bold group-focus-within:text-[#8814B1] transition-colors">
                      +998
                    </span>
                    <input
                      type="tel"
                      ...
                      className="w-full pl-16 pr-6 py-2.5 bg-gray-50 border border-gray-100 rounded-sm focus:bg-white focus:ring-4 focus:ring-purple-50 focus:border-[#8814B1] outline-none transition-all font-black text-gray-900 placeholder:text-gray-300"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-gray-900 hover:bg-[#8814B1] text-white rounded-2xl transition-all font-black text-md shadow-2xl shadow-gray-200 hover:shadow-purple-200 disabled:opacity-50 flex items-center justify-center gap-4 group"
              >
                {loading ? (
        ...
        {ordersList.length > 0 && (
          <div className="space-y-5 animate-in slide-in-from-bottom-5 duration-700">
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-black text-gray-900">
                {t.yourOrders} ({ordersList.length})
              </h2>
              <button
                onClick={() => setOrdersList([])}
                className="text-gray-400 hover:text-gray-900 font-bold text-sm flex items-center gap-1"
              >
              ...
              {ordersList.map((orderItem) => {
                ...
                return (
                  <button
                    ...
                    className="group w-full p-2.5 bg-white hover:bg-purple-50/30 rounded-2xl transition-all border border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm hover:shadow-xl hover:-translate-y-1"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-16 h-16 ${status.bg} ${status.text} rounded-2xl flex items-center justify-center shadow-sm`}
                      >
                        <Icon size={32} />
                      </div>
                      <div>
                        <p className="text-xl font-black text-gray-900 group-hover:text-[#8814B1] transition-colors">
                    ...
                    <div className="flex items-center gap-10">
                      <div className="text-right">
                        <p className="text-lg font-black text-gray-900">
                          {orderItem.totalAmount.toLocaleString()} {t.currency}
                        </p>
        ...
        {order && (
          <div className="space-y-10 animate-in slide-in-from-bottom-12 duration-700">
            ...
            <div className="bg-white rounded-[48px] p-12 shadow-sm border border-gray-50">
              <h3 className="text-2xl font-black text-gray-900 mb-12">
                {t.orderRoute}
              </h3>
                    ...
                    {getStatusSteps(order.status).map((step, index) => {
                      ...
                      return (
                        <div key={index} className="flex flex-col items-center">
                          ...
                          <p
                            className={`mt-6 text-[10px] font-black uppercase tracking-tighter ${
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-[40px] p-10 border border-gray-50 shadow-sm">
                <h4 className="text-xl font-black text-gray-900 mb-8">
                  {t.orderContents}
                </h4>
                <div className="space-y-6">
                  {order.items.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-5 p-4 bg-gray-50/50 rounded-3xl group border border-gray-50"
                    >
                        ...
                        {item.print && (
                          <div className="mt-2 flex items-center gap-2">
                            <div className="relative w-8 h-8 bg-white rounded-lg overflow-hidden border border-purple-100 shadow-xs shrink-0">
                              <Image
                                src={
                                  item.print.frontImagePreview ||
                                  item.print.frontImage
                                }
                                alt={item.print.name}
                                fill
                                sizes="32px"
              ...
              <div className="space-y-8">
                <div className="bg-gray-900 rounded-[40px] p-10 text-white shadow-2xl">
                  ...
                  <div className="space-y-1 mb-8">
                    <p className="text-gray-400 font-bold leading-tight">
                      {order.region},{" "}
                      {getTranslatedVillage(order.region, order.village)}
                    </p>
                  ...
                  <div className="bg-white/10 rounded-3xl p-6">
                    <p className="text-xs font-black text-purple-400 uppercase tracking-widest mb-2">
                      {t.contactPerson}
                    </p>
                ...
                <div className="bg-[#8814B1] rounded-[40px] p-10 text-white flex flex-col justify-between min-h-[220px] relative overflow-hidden group">
                  ...
                  <h4 className="text-xl font-black relative z-10">
                    {t.needHelp}
                  </h4>
                  <p className="text-purple-100 font-medium text-sm relative z-10 leading-relaxed">
                    {t.needHelpDesc}
                  </p>
                  <a
                    href="https://t.me/artlavkauz"
                    className="inline-flex items-center gap-3 px-8 py-4 bg-white text-[#8814B1] rounded-2xl font-black text-sm uppercase tracking-widest relative z-10 self-start mt-6 hover:shadow-xl transition-all active:scale-95"
                  >
                    {t.writeManager}
                  </a>
                </div>
