...
export default function PromotionForm({ initialData }: PromotionFormProps) {
  ...
  const [type, setType] = useState<"global" | "targeted">(initialData?.type || "global");
  const [conditionType, setConditionType] = useState<"min_items" | "min_amount" | "product_selected">(
    initialData?.conditionType || "min_items"
  );
  const [conditionValue, setConditionValue] = useState<any>(initialData?.conditionValue || "");
  ...
  const [translations, setTranslations] = useState<Record<string, { name: string; description?: string }>>(
    initialData?.translations || { ru: { name: "" }, uz: { name: "" }, en: { name: "" } }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.set("conditionValue", JSON.stringify(conditionType === "product_selected" ? (Array.isArray(conditionValue) ? conditionValue : []) : conditionValue));
    ...
    <div className="max-w-5xl mx-auto pb-20">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 px-8 py-3 bg-[#8814B1] hover:bg-[#8814B1]/90 text-white font-bold rounded-2xl transition-all shadow-lg shadow-purple-100 disabled:opacity-50 active:scale-95"
        >
          <FiSave />
      ...
      <form className="space-y-8">
        ...
        <div className="bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">Основная информация</h2>
              <p className="text-sm text-gray-500 font-medium">Настройте название и время проведения акции</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Название (RU)</h3>
                <input
                  type="text"
                  value={translations.ru.name}
                  onChange={(e) => setTranslations({...translations, ru: {...translations.ru, name: e.target.value}})}
              ...
              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Период действия</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Начало</label>
                    <input
                      type="date"
                      value={startDate}
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase mb-2">Конец</label>
                    <input
                      type="date"
                      value={endDate}
            ...
            <div className="space-y-6">
               <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Название (UZ)</h3>
                <input
                  type="text"
                  value={translations.uz?.name || ""}
                  onChange={(e) => setTranslations({...translations, uz: {...(translations.uz || {name: ""}), name: e.target.value}})}
                  className="w-full bg-white border border-gray-200 rounded-2xl px-5 py-4 text-gray-800 font-bold focus:outline-none focus:ring-4 focus:ring-purple-50 focus:border-[#8814B1] transition-all"
              ...
              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Статус</h3>
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <div className="relative">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`w-14 h-8 rounded-full transition-colors ${isActive ? "bg-[#8814B1]" : "bg-gray-300"}`}></div>
                    <div className={`absolute left-1 top-1 w-6 h-6 bg-white rounded-full transition-transform ${isActive ? "translate-x-6" : ""}`}></div>
                  </div>
                  <span className="font-bold text-gray-700">{isActive ? "Активна" : "Неактивна"}</span>
                </label>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">Условие и Механика</h2>
              <p className="text-sm text-gray-500 font-medium">Определите правила срабатывания акции</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              <div className="bg-blue-50/30 rounded-3xl p-8 border border-blue-50">
                <h4 className="text-sm font-black text-blue-900 uppercase tracking-widest mb-6">Когда срабатывает:</h4>
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-blue-400 uppercase mb-2">Область применения</label>
                    <Dropdown
                      label=""
                      options={[
                        { value: "global", label: "Глобальная (весь заказ)" },
                        { value: "targeted", label: "Целевая (конкретные товары)" },
                      ]}
                      value={type}
                      onChange={(val) => {
                        setType(val as any);
                        if (val === "targeted") setConditionType("product_selected");
                      }}
                    />
                  </div>

                  <div>
                    <label className="block text-[10px] font-black text-blue-400 uppercase mb-2">Тип условия</label>
                    <Dropdown
                      label=""
                      options={type === "global" ? [
                        { value: "min_items", label: "Мин. количество товаров" },
                        { value: "min_amount", label: "Мин. сумма заказа" },
                      ] : [
                        { value: "product_selected", label: "При выборе товаров" },
                      ]}
                      value={conditionType}
                  ...
                  {conditionType !== "product_selected" && (
                    <Input
                      label={conditionType === "min_items" ? "Количество товаров" : "Сумма в UZS"}
                      type="number"
                      value={conditionValue}
                      onChange={(e) => setConditionValue(e.target.value)}
                      placeholder={conditionType === "min_items" ? "5" : "500000"}
                      required
                    />
                  )}
                </div>
              ...
              {conditionType === "product_selected" && (
                <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden">
                  <div className="p-4 bg-gray-50 flex items-center justify-between border-b border-gray-100">
                    <span className="text-xs font-black text-gray-400 uppercase tracking-widest">Выберите товары:</span>
                    <span className="text-[10px] font-bold bg-[#8814B1] text-white px-2 py-0.5 rounded-lg">
                      Выбрано: {Array.isArray(conditionValue) ? conditionValue.length : 0}
                    </span>
                  </div>
            ...
            <div className="space-y-6">
              <div className="bg-green-50/30 rounded-3xl p-8 border border-green-50">
                <h4 className="text-sm font-black text-green-900 uppercase tracking-widest mb-6">Как рассчитывается:</h4>
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-green-400 uppercase mb-2">Вид скидки</label>
                    <Dropdown
                      label=""
                      options={[
                  ...
                  {discountType !== "free_delivery" && (
                    <Input
                      label={discountType === "percentage" ? "Процент скидки" : "Сумма скидки (UZS)"}
                      type="number"
                      value={discountValue}
                  ...
                  <div className="p-4 bg-white rounded-2xl border border-green-100 flex items-start gap-4">
                     ...
                     <div>
                        ...
                        <p className="text-[11px] text-gray-500 font-medium">
                          {type === 'global' ? 'Весь заказ' : 'Выбранные товары'} получит 
